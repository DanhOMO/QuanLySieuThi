...
public class LoginDetailDAO extends GenericDao<LoginDetail> implements ISimpleAccess<LoginDetail,Integer> {
    ...
    public boolean insert(LoginDetail loginDetail) {
        return executeUpdate("INSERT INTO LOGIN_DETAIL(AUTH_KEY,IP_ADDRESS,MAC_ADDRESS,LOGIN_TIME,LOGOUT_TIME,SOTK) VALUES(?,?,?,?,?,?)",
                loginDetail.getAuthKey(),loginDetail.getIpAddress(),loginDetail.getMacAddress(),loginDetail.getLoginTime(),loginDetail.getLogoutTime(),loginDetail.getTaiKhoan().getSoTK());
    }
    ...
    public boolean update(Integer loginId, LoginDetail loginDetail) {
        return executeUpdate("UPDATE LOGIN_DETAIL SET AUTH_KEY = ?,IP_ADDRESS = ?,MAC_ADDRESS = ?,LOGIN_TIME = ?,LOGOUT_TIME = ?,SOTK = ? WHERE LOGIN_ID = ?",
                loginDetail.getAuthKey(),loginDetail.getIpAddress(),loginDetail.getMacAddress(),loginDetail.getLoginTime(),loginDetail.getLogoutTime(),loginDetail.getTaiKhoan().getSoTK(),loginId);
    }
    ...
    public LoginDetail selectByAuthKey(String authKey) {
        return executeQuery("SELECT * FROM LOGIN_DETAIL WHERE AUTH_KEY = ? AND LOGOUT_TIME>=getDate()",authKey);
    }
}
